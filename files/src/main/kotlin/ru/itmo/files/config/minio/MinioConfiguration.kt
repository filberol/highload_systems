package ru.itmo.files.config.minio
...
class MinioConfiguration{
    ...
    fun minioClient(minioConnectionDetails: MinioConnectionDetails): MinioClient {
        ...
        if (!minioClient.bucketExists(
                BucketExistsArgs.builder().bucket(minioConnectionDetails.bucket).build()
            )
        ) {
            minioClient.makeBucket(
                MakeBucketArgs.builder().bucket(minioConnectionDetails.bucket).build()
            )
        }
        return minioClient
    }
    ...
    private class MinioConnectionDetailsImpl(val minioProperties: MinioProperties) :
